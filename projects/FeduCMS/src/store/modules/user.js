...
const actions = {
  async ac_setSchoolId({ commit }) {
    ...
    getUserInfo(userType).then((res) => {
      if (+res.code === 0) {
        let resData = res.data?.schoolIds
        if (!sessionStorage.getItem('zdyjb_schoolId')) {
          commit('setSchoolId', resData[0])
          //默认使用第一个schoolId
          sessionStorage.setItem('zdyjb_schoolId', resData[0])
          commit('setNowId')
        }
      }
    })
  ...
  async login({ commit, dispatch, rootState }, userInfo) {
    ...
    sessionStorage.setItem('schoolInfo', JSON.stringify(data.schoolInfos))
    cookie.set(`${firmId}_user_type`, data.userType)
    cookie.set(`token_${firmId}`, data.token)
    cookie.set(`${firmId}_user_id`, data.userId)
    cookie.set(`${firmId}_company_id`, data.companyId)
  ...
  async getUserInfo({ commit, dispatch, state }) {
    let userType = cookie.get(`${firmId}_user_type`)
    const res = await getUserInfo(userType)
    ...
    let { username, userHeadPortait, roleId, ability } = data.user

    dispatch('acl/setRole', [roleId], { root: true })
    //   if (ability && ability.length > 0)
    dispatch('acl/setFull', true, { root: true })
    dispatch('acl/setAbility', [], { root: true })

    commit('setUsername', username)
    commit('setAvatar', userHeadPortait)
    // dispatch('cms_code/setCode', true, { root: true })
