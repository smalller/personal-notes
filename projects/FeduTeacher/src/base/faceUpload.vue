<template>
  <div v-show="show" v-if="isModal" class="uploadMock"></div>
  <el-card
    v-if="show"
    class="upload"
  ...
  >
    <div class="uploadBox">
      ...
      <div class="uploadInfo" v-if="isModal">
        <span>{{ uploadText }}</span>
        <span>{{ schedule }}</span>
      </div>
      <div class="close" v-if="isModal">
...
export default defineComponent({
  ...
  setup() {
    ...
    const imageFileAction =
      process.env.VUE_APP_BASE_API + "/mamserver/api/file/faceUpload";
    ...
    // 上传到文件服务器
    const uploadImageFile = (file: File, studentId = "") => {
      return new Promise((resolve, reject) => {
