<template>
  <div class="top-title">
    <ChartsTitle title="幼儿数量趋势"></ChartsTitle>
    <ul>
      <li>
...
import { getMonthReport, getYearReport } from "@/api/infantManage";
  ...
  setup() {
    ...
    function pushData(item: DataItem | number) {
      if (typeof item === "number") {
        ...
        mapData.data[1].num.push(item.beforeNum);
        mapData.data[2].num.push(item.addNum);
        mapData.data[3].num.push(item.graduate);
      }
    }
    ...
    const changeData = (data: Array<DataItem>) => {
      ...
      if (+typeIndex.value === 2) {
        for (
          ...
          if (item) {
            mapData.data[0].num.unshift(item.total);
            mapData.data[1].num.unshift(item.beforeNum);
            mapData.data[2].num.unshift(item.addNum);
            mapData.data[3].num.unshift(item.graduate);
          } else {
    ...
    onMounted(() => {
      // 更新图表数据
      bus.$on("updateInfantLineData", (res: boolean) => {
        if (res) {
          // 默认展示月度数据
          typeIndex.value = 1;
          highLight.value = false;
