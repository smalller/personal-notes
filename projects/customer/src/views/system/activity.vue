    ...
    <div class="">
        <div class="container">
            <div class="handle-box">
                <el-button type="primary" icon="el-icon-edit" class="handle-del mr10" @click="addNewAct()">新增活动</el-button>
                <el-input v-model="query.text" clearable placeholder="活动主题" class="handle-input mr10"></el-input>

                <el-select v-model="query.type" placeholder="活动类型" class="handle-select mr10">
                    <!-- <el-option key="1" label="商家活动" value="1"></el-option>
                    <el-option key="2" label="音乐节" value="2"></el-option>
                    <el-option key="3" label="平台活动" value="3"></el-option> -->
                    <el-option v-for="item in activeType" :key="item.type" :label="item.text" :value="item.type"></el-option>
                </el-select>
            ...
            >
                ...
                <el-table-column align="center" prop="storeName" min-width="200" label="商家名">
                    <template slot-scope="scope">
                        <span>{{scope.row.type==1?scope.row.storeName:'-'}}</span>
                    </template>
                </el-table-column>
                <el-table-column align="center" min-width="120" label="活动类型">
                    <template slot-scope="scope">
                        <span>{{scope.row.type==1?'商家活动':(scope.row.type==2?'音乐节':'平台活动')}}</span>
                    </template>
                </el-table-column>
                <el-table-column align="center" prop="name" min-width="200" label="活动主题"></el-table-column>
                <el-table-column align="center" min-width="200" label="活动外链">
                    <template slot-scope="scope">
                        <span>{{scope.row.type==2?scope.row.link:'-'}}</span>
                    </template>
                </el-table-column>
                <el-table-column align="center" prop="createUserName" min-width="180" label="创建人"></el-table-column>
                ...
                <el-table-column prop="timeStart" label="活动开始时间" min-width="180" align="center"></el-table-column>
                <el-table-column prop="timeEnd" label="活动结束时间" min-width="180" align="center"></el-table-column>
                ...
                <!-- <el-table-column label="审核状态" align="center" min-width="180" fixed="right">
                    <template slot-scope="scope">
        ...
        <!-- 编辑弹出框 -->
        <el-dialog :title="doing" :visible.sync="editVisible" width="62%" append-to-body="true">
                    ...
                    <div class="activity">
                        <div class="in_act">
                            <el-form-item label="活动主题">
                                <el-input v-model="dynamicValidateForm.dio_name" maxlength="30" show-word-limit :disabled="!isStore"></el-input>
                            </el-form-item>
                            <el-form-item label="活动简介">
                                <el-input type="textarea" :autosize="{ minRows: 7, maxRows: 10}" v-model="dynamicValidateForm.dio_introduce" maxlength="120" :disabled="!isStore" show-word-limit></el-input>
                            </el-form-item>
                            ...
                            <el-form-item label="活动类型">
                                <el-select v-model="dynamicValidateForm.examine" placeholder="请选择活动类型" :disabled="!isStore">
                                    <el-option v-for="item in activeType" :key="item.type" :label="item.text" :value="item.type"></el-option>
                                </el-select>
                            </el-form-item>

                            <el-form-item label="活动外链" v-if="dynamicValidateForm.examine == 2">
                                <el-input placeholder="请输入链接" v-model="dynamicValidateForm.link" :disabled="!isStore" ></el-input>
                            </el-form-item>


                            <el-form-item label="活动商家" v-if="dynamicValidateForm.examine == 1">
                                <el-select v-model="dynamicValidateForm.store" filterable placeholder="请选择关联商家" :disabled="!isStore">
            ...
            <span slot="footer" class="dialog-footer">
                ...
                <el-button v-if="isStore" type="primary" @click="saveEdit">确 定</el-button>
            </span>
        </el-dialog>
    </div>
</template>
...
<script>
import {getList, getDetail, add, update, remove,storeList,fileUpload} from "@/api/system/activity";
    ...
    methods: {
        getData() {
            let data = {
                ...
                timeStart: this.query.times[0] ? this.$regular.timeData(this.query.times[0], 2) : '',
                timeEnd: this.query.times[1] ? this.$regular.timeData(this.query.times[1], 2) : '',
        ...
        lineDb(row, index, column, event) {
            ...
            row.type == 1 ? this.isStore = false : this.isStore = true
            ...
            this.$set(this.dynamicValidateForm, 'times', [row.timeStart, row.timeEnd]);
            this.$set(this.dynamicValidateForm, 'fromdata', row.banner);
            this.$set(this.dynamicValidateForm, 'thumb', row.thumb);
            this.$set(this.dynamicValidateForm, 'link', row.link);
        ...
        saveEdit() {
            let data = {
                ...
                type: this.dynamicValidateForm.examine,
                link:this.dynamicValidateForm.link || '',
                storeId:this.dynamicValidateForm.store || '0',
                audit: this.dynamicValidateForm.id ? this.dynamicValidateForm.audit : null
            };
            if (!data.name) {
                this.$message.warning(`请输入活动主题`);
                return;
            }
            if (!data.description) {
