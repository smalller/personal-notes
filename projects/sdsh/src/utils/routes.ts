 ...
 * @returns
 */
export const getJsonRouteItem = (
  pathname: string,
  jsonRoutesData: PathJsonRoutesDataItem
): RoutesDataItem => {
...
export const getRouteBelongTopMenu = (route: RoutesDataItem): string => {
  ...
  return route.parentPath
    ? route.parentPath[0] || `/${route.path.split("/")[1]}`
    : `/${route.path.split("/")[1]}`;
};
 ...
 */
export const jsonPathVueRoutes = (
  vueRoutesData: RoutesDataItem[]
...
export const hasRoutes = (
  router: RoutesDataItem[],
  route: RoutesDataItem
): RoutesDataItem => {
  for (let i = 0; i < router.length; i++) {
    const routerItem = router[i];
    if (routerItem.path === route.path) {
      route.title = routerItem.label as string;
      route.name = routerItem.label as string;
    } else if (Array.isArray(routerItem.children)) {
      for (let j = 0; j < routerItem.children.length; j++) {
        if (routerItem.children[j].path === route.path) {
          route.title = routerItem.children[j].label as string;
          route.name = routerItem.children[j].label as string;
          route.roles = routerItem.children[j].children
        }
      }
    }
  }
...
export const getPermissionMenuData = (
  ...
  for (let index = 0, len = routes.length; index < len; index += 1) {
    ...
    const routeItem = hasRoutes(router, element);
    if (routeItem.name && element.path) {
      if (element.children) {
        routeItem.children = getPermissionMenuData(router, element.children);
      }
      menu.push(routeItem);
    }
  }
...
export const equalTabNavRoute = (
  ...
  switch (type) {
    case "querypath": // path + query
      is =
        equalObject(route1.query, route2.query) && route1.path === route2.path;
      break;
    default: // path
