...
import { closeCountList } from './api'
...
export default defineComponent({
    ...
    setup(): Setup {
        ...
        async function getMerchantList(): Promise<void> {
            ...
            const { code, data } = await closeCountList(currentPage.value, pageSize.value, Object.assign({}, {
                ...QueryObj
            }));
            if (+code === 1000) {
        ...
        const selectDate = () => {
            if (selectedDate.value) {
                QueryObj.start_date = micell.date.format(selectedDate.value[0], 'YYYYMMDD');
                QueryObj.end_date = micell.date.format(selectedDate.value[1], 'YYYYMMDD');
            } else {
                QueryObj.start_date = '';
                QueryObj.end_date = '';
            }
