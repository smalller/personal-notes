...
import { bankList, subBankList } from './api'
...
export default defineComponent({
    ...
    setup(props): Setup {
        ...
        const bankCardInfoData = computed(() => props.bankCardInfoData as any) // 外界传进来的值

        if (bankCardInfoData.value) {
            dataObj.value = bankCardInfoData.value
        } else {
            const bankCardInfo = JSON.parse(localStorage.getItem('mer-bankCardInfo') as string)
            if (bankCardInfo) {
        ...
        watch(() => dataObj.value, () => {
            if (!bankCardInfoData.value) {
                localStorage.setItem('mer-bankCardInfo', JSON.stringify(dataObj.value))
            }
        }, { deep: true })
        ...
        onMounted(() => {
            if (+flag.value === 1) {
                const acntType = JSON.parse(localStorage.getItem('mer-basicInfo') as string)?.acntType  // 入账卡类型
                if (+acntType === 2) {
                    showCom.value = true
                } else {
                    showCom.value = false
