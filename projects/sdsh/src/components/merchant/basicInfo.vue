...
import { licenseTypeList, settleTpList, acntArtifFlagList } from './static'
import { PNavigationList, navigationList, productSystem, cityList, areaList, chargeRate } from './api'
...
export default defineComponent({
    ...
    setup(props) {
        ...
        const basicInfoData = computed(() => props.basicInfoData)   // 外界传进来的值
        ...
        if (basicInfoData.value) {
            ...
            // 用缓存中的数据
            const basicInfo = JSON.parse(localStorage.getItem('mer-basicInfo') as string)
            if (basicInfo) {
        ...
        watch(() => dataObj.value, () => {
            if (!basicInfoData.value) {
                localStorage.setItem('mer-basicInfo', JSON.stringify(dataObj.value))
            }
        }, { deep: true })
