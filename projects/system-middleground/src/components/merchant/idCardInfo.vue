        ...
        <div class="id-card-info-box">
            ...
            <el-form ref="idCardInfoRef" :model="dataObj" :rules="idCardInfoRules" label-width="164px">
                <ul>
                    ...
                    <li>
                        <el-form-item label="营业执照复印件：" prop="businessLicenseImg">
                            <uploadImg :readonly="+flag === 2" @uploadImg="handleImg($event, 'businessLicenseImg')"
                                @removeImg="handleImg($event, 'businessLicenseImg')" :img="dataObj.businessLicenseImg">
                            </uploadImg>
                        </el-form-item>
                    </li>
                    <li>
...
import { licenseTypeList } from '@/views/merchantManage/merchantList/static'
import { idCardInfoRules } from './static'
...
export default defineComponent({
    ...
    setup(props) {
        ...
        const idCardInfoData = computed(() => props.idCardInfoData)

        if (idCardInfoData.value) {
            dataObj.value = idCardInfoData.value
        } else {
            const idCardInfo = JSON.parse(localStorage.getItem('idCardInfo') as string)
            if (idCardInfo) {
        ...
        watch(() => dataObj.value, () => {
            if (!idCardInfoData.value) {
                localStorage.setItem('idCardInfo', JSON.stringify(dataObj.value))
            }
        }, { deep: true })
