<template>
  <el-drawer v-model="drawer" title="权限设置" custom-class="drawer-box">
    <el-tree :data="treeData" show-checkbox node-key="id" :default-checked-keys="roleCheck" :props="defaultProps"
      ref="tree" :default-expanded-keys="[]" />
...
import { getUserRole, fixUserRole } from '../api';
...
export default defineComponent({
  ...
  setup(): Setup {
    ...
    async function roleInfo() {
      treeData.value = [];
      roleCheck.value = [];
      const { code, data } = await getUserRole(roleId.value);
      if (+code === 1000) {
    ...
    async function save() {
      ...
      const { code } = await fixUserRole(params);
      if (+code === 1000) {
