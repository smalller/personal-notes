<template>
	<view @click="closeDialog">
		<navBar :navigateUrl="'../goodsManage'">编辑商品</navBar>
		...
		<view class="has-navBar">
			<view class="good-basic-info">
				<view class="left-title" :style="{'width' : goodType == 11 ? '160rpx' : '128rpx'}">
					<view class="title good-name">
						<text v-if="goodType == 1">套餐名称</text>
						<text v-else-if="goodType == 11">会员卡名称</text>
						<text v-else>商品名称</text>
					</view>
					<view class="title good-brief">
						<text v-if="goodType == 1">套餐简介</text>
						<text v-else-if="goodType == 11">会员卡简介</text>
						<text v-else>商品简介</text>
					</view>
					<view class="title" v-if="typeof putAway === 'boolean'">
						<text>是否下架</text>
					</view>
					<view class="title good-weight">
						<text>权重值</text>
					</view>
					<template v-if="goodType == 11">
				...
				<view class="right-input">
					...
					<view v-if="typeof putAway === 'boolean'">
					...
					<template v-if="goodType == 11">
						<view class="good-spec">
							<view class="input">
								<input type="number" maxlength="10" v-model="originalPrice" placeholder="请输入会员积分(积分1:1人民币)" placeholder-style="color:rgb(192, 196, 204)" />
							</view>
						</view>
						<view>
							<view class="input">
								<input type="number" maxlength="10" v-model="statisticalPrice" placeholder="请输入会员卡原价 (如：100.00)"
								 placeholder-style="color:rgb(192, 196, 204)" />
							</view>
						</view>
						<view>
							<view class="input">
								<input type="number" maxlength="10" v-model="presentPrice" placeholder="请输入会员卡现价 (如：100.00)" placeholder-style="color:rgb(192, 196, 204)" />
							</view>
						</view>
					</template>
				</view>
			...
			<view class="good-spec-wrap" v-if="goodType != 11 && goodType != 1">
				<view class="good-spec" v-for="(item,index) in goodSpecList" :key="index">
					<view class="left">
						...
						<view>
							<text>商品原价</text>
							<view class="input">
								<input maxlength="10" type="number" v-model="item.originalPrice" placeholder="请输入商品原价(如:20.00)"
								 placeholder-style="color:rgb(192, 196, 204)" />
							</view>
						</view>
						<view>
							<text>商品现价</text>
							<view class="input">
								<input maxlength="10" type="number" v-model="item.statisticalPrice" placeholder="请输入商品现价(如:20.00)"
								 placeholder-style="color:rgb(192, 196, 204)" />
							</view>
						</view>
				...
				<view class="good-spec">
					<view class="left">
						...
						<view>
							<text>商品原价</text>
							<view class="input">
								<input type="number" maxlength="10" v-model="goodPrice" placeholder="请输入商品原价(如:89.00)" placeholder-style="color:rgb(192, 196, 204)" />
							</view>
						</view>
						<view>
							<text>商品现价</text>
							<view class="input">
								<input type="number" maxlength="10" v-model="goodNowPrice" placeholder="请输入商品现价(如:89.00)" placeholder-style="color:rgb(192, 196, 204)" />
							</view>
						</view>
					</view>
			...
			<view class="combo-add-goods-box" v-if="goodType == 1">
				<view class="add-goods-box">
					<view class="add-good-box">
						<text class="title">添加商品</text>
						<view class="input">
							<input type="text" v-model="addComboGood" placeholder="请选择套餐所包含的单品" placeholder-style="color:rgb(192, 196, 204)"
							 @input="addCombo" @focus="addCombo" />
				...
				<view class="good-oldPrice">
					...
					<view class="price-input">
						<input type="number" maxlength="10" v-model="originalPrice" />
					</view>
					<!-- <text class="title">￥{{originalPrice}}</text> -->
				</view>
				<view class="good-nowPrice">
					<text class="title">套餐现价</text>
					<view class="price-input">
						<input type="number" maxlength="10" v-model="comboNewPrice" />
					</view>
				</view>
			</view>
			...
			<view class="good-images">
				<view class="good-img-main">
					...
					<view class="img-show" v-if="goodType != 11 && goodShowImg">
					...
					<view class="file-upload" v-if="goodType != 11 && !goodShowImg" @click="chooseImage(1,'480upx','480upx')"></view>


					<view class="vip-card-box" v-if="goodType == 11">
				...
				<view class="good-img-detail" v-if="goodType != 11">
					<view class="title">
						<text>商品详情图</text>
				...
				<view class="good-img-recom" v-if="goodType != 11">
					<view class="title">
						<text>商品推荐位</text>
...
<script>
	...
	export default {
		...
		methods: {
			...
			//在已经返回的单品列表里搜索单品
			inputSelectCombo() {
				const index = this.comboSelectGoodsList.indexOf(this.addComboGood);
				if (index !== -1) {
			...
			changeGoodNum() {
				//计算套餐原价（所有单品原价之和）
				this.originalPrice = this.comboGoodsList.map(item => item.originalPrice * item.number).reduce((prev, item) => prev +
					item, 0).toFixed(2);

				//计算套餐现价（所有单品现价之和）
				if (this.updateComboNewPrice) {
					this.comboNewPrice = this.comboGoodsList.map(item => item.presentPrice * item.number).reduce((prev, item) => prev +
						item, 0).toFixed(2);
				}
			},
			...
			checkFormInfo() {
				if (!this.goodName) {
					...
					return;
				} else if (!this.goodDetailImg && this.goodType != 11) {
			...
			handleUpdate() {
				//获取规格中最低的商品售价（原价）
				if (this.goodType != 11 && this.goodType != 1) this.getOriginalPrice();
				...
				let data = {
					...
					listPicture: this.goodShowImg, //商品图
					infoPicture: this.goodDetailImg, //商品详情图
					recommendPicture: this.goodRecommImg, //商品推荐位
					// recommendAdPicture : this.goodBannerImg,	//店铺宣传图
					...
					deleteSkuList: this.deleteSkuList, //删除的规格id数组
					statisticalPrice: this.goodType == 1 ? this.comboNewPrice : this.goodType == 11 ? this.statisticalPrice : '',
			...
			deleteGoodSpec(item) {
				...
				//删除一个现有的规格就把sku码添加到这个删除规格数组里
				if (item.skuCode !== '') {
					this.deleteSkuList.push(item.skuCode);
				}
			},
			...
			getGoodDetails(id) {
				this.$get(`/merchant/store/goods/getGoodsInfo/${id}`).then((result) => {
					if (result.code === 0) {
						let res = result.data;
						this.id = res.id;
						this.putAway = res.putAway === 1 ? true : false;
						this.goodType = res.type;
						this.goodName = res.name;
						this.goodBrief = res.synopsis;
						this.goodWeight = res.goodsSort;
						this.goodSpecList = res.skuList;
						this.goodRecomSwitch = res.recommendStatus === 1 ? true : false;
						this.goodShowImg = res.listPicture
						this.goodDetailImg = res.infoPicture;
						this.goodRecommImg = res.recommendPicture;
						// this.goodBannerImg = res.recommendAdPicture;
						this.presentPrice = res.presentPrice; //会员卡现价
						this.originalPrice = res.originalPrice; //会员卡积分/套餐单品原价之和
						this.comboGoodsList = res.setMealGoodsList; //当前套餐已有的单品集合
						this.comboNewPrice = res.statisticalPrice; //套餐现价
						this.statisticalPrice = res.statisticalPrice; //会员卡现价

						//将售罄的number值转为bool值
						this.goodSpecList.forEach(item => {
							item.sellOut = item.sellOut === 1 ? true : false;
						})

						//将套餐列表里的单品id传到已选择单品id数组中
						this.comboGoodsIdList = this.comboGoodsList.map(item => item.goodsId);

						//获取会员卡卡片列表
						res.type == 11 && this.getVipCardList();
						
						console.log('vvvv',this.goodSpecList)
					}
				});
			},
