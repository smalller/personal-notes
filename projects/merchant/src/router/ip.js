...
import MessageE from "element-ui/packages/message/index.js";
...
if (process.env.NODE_ENV === "development") {
  api = "/api";
  tengxun = "/map";
  imgHead = '/api/admin/system/upload/down?keyName=';
...
file_request.interceptors.request.use(config => {
  NProgress.start();
  config.headers['Authorization'] = `Basic ${Base64.encode(`${website.clientId}:${website.clientSecret}`)}`;
  // config.headers['Authorization'] = `Basic ${website.clientSecret}`;
