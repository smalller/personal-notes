<template>
  <div class="avue-top">
    ...
    <div class="top-bar__right">
      <span style="font-size: 16px">当前店铺：</span>
      ...
      <img class="top-bar__img" :src="showImgPrefix + userInfo.avatar" />
      <el-dropdown>
...
import { changeStore } from "@/api/system/changeStore.js";
...
export default {
  ...
  filters: {},
  created() {
    this.roles = JSON.parse(localStorage.getItem("store-info")) || "";
  },
  mounted() {
  ...
  methods: {
    //切换店铺操作
    async changeStore() {
      const res = await changeStore(this.storeId);
      if (res.data.code === 200) {
        this.$message.success("切换店铺成功");
        this.roles.forEach((item) => {
          if (item.storeId == this.storeId) {
            localStorage.removeItem("present-store-info");
            localStorage.setItem("present-store-info", JSON.stringify(item));
            location.reload();
          }
        });
      }
