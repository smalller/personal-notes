  ...
  <div id="coupons">
    ...
    >
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="saveBefore" v-if="!readOnly"
          >确定</el-button
        >
      </span>
    </el-dialog>
  ...
  methods: {
    getData() {
      let data = {
        ...
        timeStart: this.query.begin_end
          ? this.$regular.timeData(this.query.begin_end[0], 2)
          : "",
        timeEnd: this.query.begin_end
          ? this.$regular.timeData(this.query.begin_end[1], 2)
          : "",
      };
    ...
    enable(index, val) {
      ...
      enableFun(data).then((res) => {
        if (res.data.code == 200) {
          this.$message.success(`修改成功`);
          this.$set(val, "status", val.status == 0 ? 1 : 0);
        }
      });
    },
    // 新增/修改  保存
    save() {
      ...
      from.forEach((i) => {
        ...
        let data = {
          timeStart: this.$regular.timeData(i.start_end[0], 2),
          type: +this.form.region,
          moneyMin: Number(i.full),
          details: str,
          moneyDiscount: Number(i.minus) || 0,
          id: id,
          timeEnd: this.$regular.timeData(i.start_end[1], 2),
          numLeft: Number(i.give),
          source: 0,
          numTotal: Number(i.give),
          timeStop: this.$regular.timeData(i.offTime, 2),
          status: 0,
          gift: i.minus,
        };
        if (!this.editRead) {
