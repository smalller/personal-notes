  ...
  <div id="map">
    <div class="seac_address" v-loading="cityTrue">
      <span @click.stop class="pro_add">
        <el-input
          v-model="fristForm.address"
          placeholder="请输入地址"
          @focus.stop="showFun(1)"
        ></el-input>
      </span>
      ...
      <ul
        :class="
          mapList.merchantType === 1
            ? 'add_list merchant-type1'
            : 'add_list merchant-type2'
        "
        v-if="showList"
      >
        <div v-if="addressLists != ''">
          <li
            @click="assignText(item)"
      ...
      <div
        id="container"
        ref="container"
        :style="{ width: mapWidth, height: maiHeight }"
      ></div>
    </div>
  </div>
</template>
...
export default {
  ...
  mounted() {
    ...
    let geolocation = new qq.maps.Geolocation(
      "UXBBZ-RELWI-JJXG2-5YBRI-XPWW3-D3FIJ",
      "dingzuo"
    );
    geolocation.getLocation(this.showPosition, this.showPositionErr, {
      timeout: 20000,
    });
  },
  watch: {
  ...
  methods: {
    ...
    showPosition(position) {
      ...
      this.fristForm.longitude =
        this.mapList.longitude === -1 ? position.lng : this.mapList.longitude;
      this.fristForm.latitude =
        this.mapList.latitude === -1 ? position.lat : this.mapList.latitude;
    ...
    TMap() {
      return new Promise(function (resolve, reject) {
        ...
        script.src =
          "https://map.qq.com/api/js?v=2.exp&callback=init&key=UXBBZ-RELWI-JJXG2-5YBRI-XPWW3-D3FIJ";
      ...
      this.TMap().then((qq) => {
        ...
        geocoder = new qq.maps.Geocoder({
          complete: function (result) {
            ...
            var gps =
              result.detail.location.lat + "," + result.detail.location.lng;
            var marker = new qq.maps.Marker({
              map: map,
