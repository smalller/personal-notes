            ...
            <div>
              ...
              <div class="list-box add-drinks">
                ...
                >
                  ...
                  <div v-if="isHasGetTable">
                    <el-button
                      @click="getTable(item.orderUserId)"
                      v-if="
                        orderDetailInfo.statusOrder == 5 && item.status == 0
                      "
                      type="primary"
                      >上桌</el-button
                    >
...
<script>
import { append } from "@/api/store/order";

export default {
  props: ["orderDetailInfo", "isHasGetTable"],

  methods: {
    async getTable(orderUserId) {
      const res = await append(orderUserId);
      if (res.data.code === 200) {
        this.$message.success("上桌成功");
