                            ...
                            <el-tabs v-model="activeName" @tab-click="handleClick">
                                ...
                                <el-tab-pane label="联系官方" name="service">
                                    <div class="sves" v-for="(item, i) in service" :key="i" @click="createService(item)">
                                        <img :src="imgHead + item.userPortrait" alt="" />
...
import {getUserInfo,fileUpload,allSystemUser} from "@/api/store/chatRoom";
export default {
    data() {
        return {
            loading: false,
    ...
    watch: {
        break: {
            handler() {
                let rToken = JSON.parse(localStorage.getItem('userInfo')).rongToken;
                let userInfo = {
    ...
    methods: {
        ...
        lookOrder(val) {
            if (val.content.kind == 'JZ:MessageVieOrder') {
                ...
                if (this.$route.path != '/store/order') {
                    this.$store.commit('onloadOrderFun', (this.$store.state.onloadOrder += 1));
                ...
                if (res.data.code == 200) {
                    ...
                    list.forEach((v, i) => {
                        ...
                        if (v.messageDirection == 2) {
                            ...
                            v.content['name'] = this.selfInfo.nick_name;
        ...
        createService(service) {
            ...
            let newUser = true;
            for (let i = 0; i < this.userList.length; i++) {
                if (this.userList[i].targetId == service.userId) {
                    newUser = false; //找到一个相同  证明是已出现过
                    break;
                }
            }
            lastObj['id'] = service.userId;
            lastObj['targetId'] = service.userId;
            lastObj['name'] = service.userName;
            lastObj['portrait'] = this.imgHead + service.userPortrait;
            if (newUser) {
                // 获取未读消息条数
                var conversationType = RongIMLib.ConversationType.PRIVATE;
