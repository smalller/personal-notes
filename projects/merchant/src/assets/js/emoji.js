/*
    支持范围
    IE7+, Chrome, Safari, Firefox, Android, IOS
 */
(function(global, factory) {
	...
	var isMini = isMiniEnv(window || global);
	...
	var DEFAULT_LANG = 'zh';
	// 支持的语言
	var SUPPORT_LANGS = ['zh', 'en'];
	// 24px 的 emoji 图片地址
	var NORMAL_IMAGE_PATH = '//cdn.ronghub.com/emojis-normal.png';
	// 48px(高清图) 的 emoji 图片地址
	var HD_IMAGE_PATH = '//cdn.ronghub.com/emojis-hd.png';
	...
	var Utils = {
			...
			symbolToText: function(symbol) {
					return symbol.substr(1, symbol.length - 2);
			},
			textToSymbol: function(text) {
					var tpl = '[{{text}}]';
	...
	function addRecent(emoji, count) {
			...
			if (hasMatched) {
					...
					recentList.push({
							...
							emoji: emoji,
							node: (isAppPlus || isMini) ? null : Utils.getDom(html),
