...
const TransactionItem = (props: ListDetails) => {
  return (
    <div
      ...
      onClick={() => {
        history.push({
          pathname: '/parent/transactionDetails',
          query: {
            id: props.id,
          },
        });
...
const TransactionBox = (props: ListItem) => {
  return (
    <div className={'transactionItem'}>
      <div className={'dateBox'}>{dayjs(props.time).format('YYYY年MM月')}</div>
      <div className={'detailsList'}>
        {props.parentDetailsVos.map((item) => (
          <TransactionItem
            id={item.id}
            key={item.id}
      ...
      <div className={'content'}>
        {props.list.map((item, index) => (
          <TransactionBox
            key={index}
            time={item.time}
            parentDetailsVos={item.parentDetailsVos}
          />
        ))}
      </div>
